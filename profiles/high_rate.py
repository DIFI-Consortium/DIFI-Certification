...
def validate_profile_context(profile, parsed):
    errors = []
    if parsed.sampleRate != 625e6:
        errors.append("Sample rate doesn't match profile (expected 625e6)")
...
def validate_profile_data(profile, parsed):
    errors = []
    if parsed.header.pktSize != 2232:
        errors.append("Expected data pktSize=2232")
    return errors
